  ...
  return (
    <section className="flex h-full min-h-0 flex-col rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800/40">
      <div className="border-b border-slate-100 px-4 py-3 dark:border-slate-700">
        ...
        <p className="mt-0.5 text-xs text-slate-500">Görev dağılımı ve ilerleme</p>
      </div>
      <div className="flex-1 space-y-2 p-4">
            ...
            >
              <div className="flex items-center gap-2">
                <Icon
                  className={cn(
                    "h-4 w-4",
                    row.tone === "success" && "text-emerald-600",
                    row.tone === "warning" && "text-amber-600",
                    row.tone === "danger" && "text-red-600",
                    (!row.tone || row.tone === "default") && "text-slate-400"
                  )}
                  aria-hidden
                />
        ...
        <div className="rounded-lg border border-emerald-200 bg-emerald-50/70 px-3 py-3 dark:border-emerald-800 dark:bg-emerald-950/30">
          ...
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-emerald-200/70">
            <div className="h-full rounded-full bg-emerald-500" style={{ width: `${kpis.completionPct}%` }} />
          </div>
        </div>
      </div>
      <div className="border-t border-slate-100 p-3 dark:border-slate-700">
        <Button
          ...
          variant="outline"
          size="sm"
          className="h-9 w-full gap-1.5 border-slate-200 text-xs"
